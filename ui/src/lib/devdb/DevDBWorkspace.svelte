...
{/if}
<style>
  .workspace { display: grid; grid-template-columns: minmax(280px, 320px) minmax(0, 1fr); min-height: 430px; border: 1px solid var(--border); border-radius: var(--radius-lg); overflow: hidden; background: var(--card); } nav { border-right: 1px solid var(--border); min-width: 0; } main { display: grid; grid-template-columns: minmax(0, 1.15fr) minmax(0, 1fr); align-items: start; gap: var(--space-4); padding: var(--space-4); min-width: 0; } .scene { min-width: 0; position: sticky; top: var(--space-4); } .empty { padding: var(--space-6); text-align: center; border: 1px dashed var(--border); border-radius: var(--radius-lg); color: var(--dim); } .empty p { color: var(--dim); }
