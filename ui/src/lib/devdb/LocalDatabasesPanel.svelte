...
{#if devStore.dbProjects.length === 0}<p class="empty">No SQL projects are configured for this environment.</p>{:else}{#each devStore.dbProjects as project (project.path)}<h4>{project.name}</h4><DatabaseOperationsList {project} states={devStore.dbState} operation={devStore.dbOpInFlight} {disabledReason} variant="drawer" />{/each}{/if}
