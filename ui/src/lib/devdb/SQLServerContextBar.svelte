<script lang="ts">
  type Props = { health: string; environmentName?: string; onOpenService: () => void }
  let { health, environmentName, onOpenService }: Props = $props()
...
{:else}
  <section class="degraded" role="status"><strong>SQL Server is {health || 'not healthy'}.</strong><span>Schema checks, publishing, and resets are unavailable.</span><button type="button" onclick={onOpenService}>Open SQL Server</button></section>
{/if}
<style>
  .context { display: grid; grid-template-columns: auto 1fr; align-items: center; border: 1px solid var(--border); border-radius: var(--radius-lg); background: var(--card); overflow: hidden; }
