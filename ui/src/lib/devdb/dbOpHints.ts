...
const HINTS: Array<{ re: RegExp; hint: string }> = [
  {
    re: DATA_LOSS_RE,
    hint: 'blocked to avoid data loss — inspect the change, then use `orbit db publish <db> --force` if it is intentional; use `orbit db reset <db>` to discard all local data',
  },
  {
    re: /\.dacpac.*(not exist|not found|could not be)|references? .*dacpac/i,
    hint: 'a referenced dacpac is missing — fix the project build, then run `orbit db publish --all` for first-time setup',
  },
  {
