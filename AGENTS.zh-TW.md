...
- 破壞性操作：`docker volume rm`、`orbit db reset` / `orbit db publish --clean`（把 DB 還原到 baseline snapshot，會丟掉 baseline 之後的資料變更）、`orbit db publish --force`（允許可能造成資料遺失的 schema 變更）。舊的 image-build flow（`orbit db build` 與 container 端 apply）已移除；publish / reset 全程在 host 上執行。
