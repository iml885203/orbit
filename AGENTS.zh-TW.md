...
- 破壞性操作：`docker volume rm`、`orbit sqlserver reset`（把單一 DB 回復到最新 schema 的乾淨狀態，會丟掉本機資料變更）、`orbit sqlserver publish --allow-data-loss`（允許可能造成資料遺失的 schema 變更）。舊的 image-build flow 與 container 端 apply 已移除；publish / reset 全程在 host 上執行。
