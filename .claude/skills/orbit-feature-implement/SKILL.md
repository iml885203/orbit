...
description: "Implement an Orbit feature request end to end from a required GitHub issue number or URL: validate that the issue is an open, sufficiently clear, independently verifiable feature request; create an isolated git worktree; implement and test it; run Orbit code review with sub-agents; open a PR; resolve review and CI failures; merge and close the issue; update local main; and perform exploratory testing. Use when the user asks to implement an Orbit feature issue through PR and merge, including prompts such as `/orbit-feature-implement #13`, `implement feature issue 13`, or `實作這個 feature request issue URL`. Do not use for bugs, chores, refactors, or requests without an issue identifier."
---

# Orbit Feature Implement

Drive one well-defined Orbit feature request from issue intake through post-merge exploratory verification. Keep the user informed at meaningful milestones and persist until the workflow finishes or reaches a genuine stop condition.
...
2. It requests user-visible or developer-facing new capability, rather than a bug fix, refactor, maintenance task, investigation, or open-ended discussion. Treat labels as evidence, not the sole classification signal.
...
4. Establish a baseline by running the relevant existing tests. If the baseline fails, determine whether the failure is pre-existing. Stop and report it when it prevents reliable feature verification.
...
2. Add behavioral tests that fail without the feature and cover important boundary or error cases. For UI behavior, verify the user-visible interaction, not only helper functions.
...
2. Push the feature branch without force and open a non-draft PR. Include the issue linkage (`Closes #<number>`), acceptance criteria, implementation summary, and exact verification performed.
...
4. Exercise the feature from the user's perspective on the updated default branch. Cover the primary path, at least one meaningful boundary or failure path, and interaction with the closest existing behavior. Use Orbit's real CLI/UI/runtime where feasible; use `--json` whenever parsing Orbit CLI output.
...
6. Remove the feature worktree only after the merge and post-merge verification succeed, and only when it contains no uncommitted changes. Delete the merged local feature branch when safe.
